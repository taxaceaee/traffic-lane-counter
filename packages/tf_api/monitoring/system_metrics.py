        ...
        if torch.cuda.is_available():
            ...
            try:
                ...
                try:
                    ...
                    logger.debug("GPU memory fallback failed", exc_info=True)
    except ImportError:
        logger.debug("Torch is not installed; GPU metrics unavailable")
    return result
...
def _collect_snapshot() -> dict[str, Any]:
    ...
    try:
        from tf_api.services.health_checker import check_database, check_redis
        ...
        logger.debug("Dependency health snapshot failed", exc_info=True)
    ...
    try:
        ...
        logger.debug("Live camera metrics snapshot failed", exc_info=True)
    try:
        from tf_api.api.routes_cameras import get_camera_list
        all_cams = get_camera_list()
        total_cameras = len(all_cams)
    except Exception:
        logger.debug("Camera list snapshot failed", exc_info=True)

    ws_connections = 0
    try:
        from tf_api.api.routes_ws import get_connection_stats
        stats = get_connection_stats()
        ws_connections = stats.get("global_connections", 0)
    except Exception:
        logger.debug("WebSocket connection snapshot failed", exc_info=True)

    active_jobs = 0
    total_jobs = 0
    try:
        from tf_api.api.routes_jobs import get_job_stats
        job_stats = get_job_stats()
        active_jobs = job_stats.get("active", 0)
        total_jobs = job_stats.get("total", 0)
    except Exception:
        logger.debug("Job statistics snapshot failed", exc_info=True)
