...
def _emit_alert(severity: str, title: str, message: str, alert_type: str = "general", camera_id: str | None = None) -> None:
    try:
        ...
        logger.debug("Failed to emit health alert", exc_info=True)
...
def _resolve_alert(alert_type: str, camera_id: str | None = None) -> None:
    try:
        ...
        logger.debug("Failed to resolve health alert", exc_info=True)
...
def check_all() -> dict[str, Any]:
    ...
    blocking = [k for k in unhealthy if k == "database" or (k == "redis" and redis_required)]
    if redis_required and deps["redis"].get("status") != "healthy":
        blocking.append("redis")
    return {
        "status": "ready" if not blocking else "unready",
        "dependencies": deps,
        "unhealthy": unhealthy if unhealthy else None,
        "blocking": sorted(set(blocking)) or None,
    }
