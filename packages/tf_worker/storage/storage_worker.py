...
def _emit_queue_alert(camera_id: str, qsize: int, maxsize: int) -> None:
    try:
        ...
        logger.debug("Failed to emit storage backpressure alert", exc_info=True)
...
class StorageWorker:
    ...
    def __init__(
        ...
        if adapter is not None and adapter_factory is not None:
            raise ValueError("Pass adapter or adapter_factory, not both")
        if adapter is not None:
            self.adapter = adapter
        else:
            self.adapter = None
        self._adapter_factory = adapter_factory or db_session_factory
    ...
    def enqueue(
        ...
        *,
        ...
        if qsize >= _QUEUE_CRIT_THRESHOLD:
            ...
            self._append_dead_letter(payload)
    ...
    def enqueue_lane_change(
        ...
        *,
        ...
        """Queue a stable-lane change for durable storage and realtime publish."""
        payload = {
            "kind": "lane_change",
            "camera_id": camera_id,
            "job_id": job_id,
        ...
        try:
            ...
            self.dropped_events += 1
            self._append_dead_letter(payload)
            logger.warning("StorageWorker queue full — dropping lane-change frame_id=%d", frame_id)
    ...
    def _append_dead_letter(self, payload: dict[str, Any]) -> None:
