...
class LineCounter:
    ...
    def update_lines(self, lanes_data: list[dict]) -> None:
        """Hot-swap counting lines without resetting cumulative counts.

        Preserves existing count data and last_side state in track states.
        Existing lines keep their counters; new lines start at zero;
        removed lines' counters are kept (orphan data) — caller may
        discard stale line_ids via get_counts().
        """
        new_lines = []
        for lane in lanes_data:
            ...
            if not cl:
                continue
            lane_id = lane["id"] if "id" in lane else lane.get("lane_id", "")
            line = CountingLine(
                line_id=f"{lane_id}_count",
                lane_id=lane_id,
                start=cl["start"],
            ...
            # Ensure count bucket exists for this lane
            bucket = self.counts.setdefault(lane_id, {})
            for cls_name in self._allowed_classes:
                if cls_name not in bucket:
                    bucket[cls_name] = {d: 0 for d in self.DIRECTIONS}
            if not bucket and self._allowed_classes:
                bucket[self._allowed_classes[0]] = {d: 0 for d in self.DIRECTIONS}
