...
async function loadUsersData() {
    ...
    tbody.innerHTML = users.map(u => {
        ...
        return `<tr class="border-b border-slate-800/50">
            <td class="px-4 py-3 font-medium text-white">${escapeHtml(u.username)}</td>
...
async function loadAuditData() {
    ...
    tbody.innerHTML = logs.map(l => `
        <tr class="border-b border-slate-800/50">
            ...
            <td class="px-4 py-3 text-white font-medium">${escapeHtml(l.username)}</td>
            <td class="px-4 py-3"><code class="text-xs bg-slate-800 px-1.5 py-0.5 rounded text-indigo-300">${escapeHtml(l.action)}</code></td>
            <td class="px-4 py-3 text-slate-300 text-xs">${escapeHtml(l.resource || '—')}</td>
            <td class="px-4 py-3 text-slate-400 text-xs">${escapeHtml(l.detail || '—')}</td>
        </tr>
    `).join('');
}
