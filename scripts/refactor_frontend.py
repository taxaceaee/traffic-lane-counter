...
extract JS into frontend/js/app.js, and rewrite index.html to load dynamically.
"""
import os, re
...
def main():
    ...
    # Extract <script> into js/app.js
    sm = re.search(r'(?s)<script>\s*\n(.*?)</script>', html)
    if sm:
        write('js/app.js', sm.group(1))
        html = html.replace(sm.group(0), '<script src="js/app.js"></script>')
