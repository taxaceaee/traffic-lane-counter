...
def _require_admin(cred: HTTPAuthorizationCredentials | None):
    ...
    from backend.api.routes_auth import ALGORITHM, SECRET_KEY
    try:
...
async def system_health(cred: HTTPAuthorizationCredentials | None = Depends(security)):
    ...
    _require_admin(cred)
    from backend.monitoring.system_metrics import get_latest
    return get_latest()
...
async def system_health_history(
    ...
    _require_admin(cred)
    from backend.monitoring.system_metrics import get_history
