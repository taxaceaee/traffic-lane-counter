...
from shared.config.compiler import ConfigError, compile_camera_config, write_compiled_config
from shared.config.loader import resolve_path
...
# Load from settings service so operators can tune via Settings UI
try:
    from backend.services.settings_service import get_system_config
    _sys_cfg = get_system_config()
    _MAX_CONCURRENT_JOBS = max(1, min(16, _sys_cfg.get("max_workers", 4)))
except Exception:
