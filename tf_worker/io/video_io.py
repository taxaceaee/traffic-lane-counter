    ...
    def _capture_loop(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            if not success:
                ...
                sleep_time = reconnect_delay + random.SystemRandom().uniform(-jitter, jitter)
                logger.warning(
...
class YouTubeLiveReader:
    ...
    def _setup_one_shot(self) -> None:
        ...
        from tf_common.yt_utils import download_video as _download_yt
    ...
    def _reopen_capture(self) -> None:
        """Re-open the current resolved live URL after read/open failure."""
        stream_url = self._stream_info.get("url")
        if not stream_url:
            return
        ...
        if not new_cap.isOpened():
            ...
            return
        input_cfg = self._config.get("input", {})
        new_cap.set(cv2.CAP_PROP_BUFFERSIZE, input_cfg.get("buffer_size", 1))
        old_cap = self._capture
        self._capture = new_cap
