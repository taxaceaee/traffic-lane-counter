...
class TrackState:
    ...
    __slots__ = (
        "track_id", "class_name", "first_seen_frame", "last_seen_frame",
        "bbox", "confidence", "raw_history", "stable_lane",
        "consecutive_unknown_count", "last_side",
    )

    def __init__(self, track_id: int, class_name: str, first_seen_frame: int, history_window: int = 10):
        ...
        # History of raw lane classifications — deque avoids O(n) pop(0)
        self.raw_history: deque[str] = deque(maxlen=history_window)
    ...
    def update(self, current_frame_idx: int, detected_tracks: list[dict[str, Any]], lane_assigner: LaneAssigner) -> list[dict[str, Any]]:
        ...
        for track in detected_tracks:
            ...
            if tid not in self.track_states:
                # New track detected
                self.track_states[tid] = TrackState(tid, cls_name, current_frame_idx, self.history_window)
            ...
            # Update raw history — deque auto-trims at maxlen
            ...
            if raw_lane == "unknown":
                ...
                last_C = list(state.raw_history)[-C:] if len(state.raw_history) >= C else []
