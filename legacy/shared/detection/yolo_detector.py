...
class YoloDetectorWrapper:
    ...
    def __init__(self, weights_path: str | Path, half: bool = False):
        ...
        # ── cuDNN auto-tuner ──────────────────────────────────────────────
        # Enable benchmark mode: once a fixed input size is seen, cuDNN
        # benchmarks all available kernels and caches the fastest one.
        # Without this, the default kernel is used which can be 10-15% slower.
        torch.backends.cudnn.benchmark = True
        torch.backends.cudnn.allow_tf32 = True  # TF32 matmul on Ampere+
        ...
        # The first prediction is 2-5× slower because PyTorch compiles
        # CUDA kernels and cuDNN benchmarks convolution algorithms.
        # Run 3 dummy frames now so every real frame is fast.
        if torch.cuda.is_available() and self.model.task == "detect":
            import numpy as np
            imgsz = self.model.model.args.get("imgsz", 640)
            dummy = np.zeros((imgsz, imgsz, 3), dtype=np.uint8)
            for _ in range(3):
                self.model.predict(dummy, imgsz=imgsz, conf=0.5, verbose=False)
            del dummy
            logger.info("Model warmup complete: 3 dummy runs for %s", self.weights_path)

        # NOTE: torch.compile() was attempted here but causes graph breaks
        # with Ultralytics YOLO's batch_norm layers in PyTorch 2.12 + fp16
        # (batchnorm FakeTensor device propagation issue).
        # TensorRT export via ``model.export(format='engine', half=True)``
        # is recommended instead for an additional 1.5-2× speedup.
