...
class CropROI:
    ...
    def update_zones(
        self,
        zone_polygons: list[list[list[float]]] | None,
        lanes_config: list[dict[str, Any]] | None = None,
    ) -> tuple[int, int]:
        """Hot-update crop region from new zone polygons without recreating CropROI.

        Returns (old_offset_x, old_offset_y) so caller can re-offset coordinates.
        """
        old_ox, old_oy = self.offset_x, self.offset_y

        source_polygons: list[list[list[float]]] | None = zone_polygons
        if not source_polygons and lanes_config:
            source_polygons = [
                lane.get("points", [])
                for lane in lanes_config
                if len(lane.get("points", [])) >= 3
        ...
        for poly in source_polygons or []:
            for pt in poly:
                x, y = float(pt[0]), float(pt[1])
                x_min = min(x_min, x)
                y_min = min(y_min, y)
        ...
        if x_min == float("inf"):
            ...
            self.x1 = max(0, int(x_min) - self.padding)
            self.y1 = max(0, int(y_min) - self.padding)
            self.x2 = min(self.frame_width, int(x_max) + self.padding)
            self.y2 = min(self.frame_height, int(y_max) + self.padding)
    ...
    def to_original(
        ...
        # to a 1×1 box at that edge so downstream code (crop extraction,
