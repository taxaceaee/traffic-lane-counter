...
from backend.io.image_sequence import ImageSequenceReader
...
class YouTubeLiveReader:
    """YouTube video/live-stream reader with auto URL refresh and robust reconnection.
    ...
    Thread safety:
      - ``self._capture`` and ``self._queue`` are protected by ``self._lock``.
        Both ``_capture_loop`` and ``_swap_capture`` acquire it before touching
        either reference, preventing TOCTOU bugs on released capture handles.
    ...
    def __init__(self, source: str, config: dict):
        ...
        self._lock = threading.Lock()  # protects _capture + _queue
        self._stop_event = threading.Event()
    ...
    def _resolve_stream_info(self) -> dict:
        ...
        # Fulfil the return contract — yt_utils already returns same keys
        return dict(info)  # shallow copy is fine
    ...
    def _capture_loop(self) -> None:
        ...
        On read failure (transient YouTube HLS blip or token expiry), waits
        briefly and re-resolves the stream URL via yt-dlp before retrying.
        Thread uses ``self._lock`` to access ``self._capture`` and
        ``self._queue`` safely alongside ``_swap_capture``.
        """
        min_interval = (1.0 / self._target_fps) if self._target_fps else 0.0
        last_read = 0.0
        reconnect_delay = 1.0
        ...
        while not self._stop_event.is_set():
            ...
            if not success:
                ...
                jitter = reconnect_delay * 0.3
                sleep_time = reconnect_delay + random.uniform(-jitter, jitter)
                logger.warning(
                    "YouTubeLiveReader: read failed for %s — reconnecting in %.1fs",
                    self._source_url, sleep_time,
                )
                time.sleep(max(0.1, sleep_time))
                ...
                try:
                    ...
                    logger.warning("YouTubeLiveReader: reconnect resolve failed", exc_info=True)
    ...
    def _swap_capture(self, new_url: str) -> None:
        ...
        Uses ``self._lock`` to synchronise with ``_capture_loop`` on every
        ``self._capture`` and ``self._queue`` access.
        """
        with self._lock:
            if self._queue:
                try:
