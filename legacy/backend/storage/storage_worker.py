...
from backend.services.circuit_breaker import CircuitBreakerOpen, db_breaker
from backend.storage.crop_protocol import CropStorage
from backend.storage.pubsub_protocol import StreamPublisher
...
# Watchdog thresholds — loaded from settings.json so operators can tune via Settings UI
try:
    from backend.services.settings_service import get_notification_thresholds
    _nt = get_notification_thresholds()
    _QUEUE_WARN_THRESHOLD = max(64, min(4096, _nt.get("backpressure_warn_threshold", 512)))
    _QUEUE_CRIT_THRESHOLD = max(128, min(8192, _nt.get("backpressure_crit_threshold", 1024)))
    _DEAD_LETTER_MAX = max(100, min(100000, _nt.get("dead_letter_max", 10000)))
except Exception:
...
class StorageWorker:
    ...
    def __init__(
        ...
        if crop_storage is not None:
            ...
            from backend.storage.local_crop_storage import LocalCropStorage
