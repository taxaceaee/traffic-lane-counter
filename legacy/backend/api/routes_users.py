"""Users API — DB-backed user management.

Admin-only: all endpoints require admin JWT (except login which is in auth).
...
from backend.api.routes_auth import get_current_user, _hash_password
...
def _audit(user_id, username, action, resource="", detail="", ip=""):
    """Write audit entry in a fresh session."""
    try:
...
async def list_users(_user: dict = Depends(get_current_user)):
    """List all users. Any authenticated user can list."""
...
async def create_user(body: CreateUserRequest,
    ...
    try:
        ...
        _audit(user["sub"], user["sub"], "user_create",
...
async def update_user(user_id: str, body: UpdateUserRequest,
    ...
    try:
        ...
        _audit(user["sub"], user["sub"], "user_update",
...
async def delete_user(user_id: str, user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        _audit(user["sub"], user["sub"], "user_delete",
