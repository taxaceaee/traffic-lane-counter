...
Log level resolution (first win):
  1. LOG_LEVEL env var (containerised deployments)
  2. configs/settings.json → system.log_level
  3. INFO (default)
...
def _resolve_log_level() -> str:
    """Resolve log level: env var > settings.json > INFO."""
    env_level = os.getenv("LOG_LEVEL", "").upper()
    if env_level in ("DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"):
        return env_level
    try:
        from backend.services.settings_service import get_log_level
